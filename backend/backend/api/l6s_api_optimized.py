...
def translate_text(text, target_lang, brain):
    ...
    if lang == "fr":
        return text
    # Avoid unnecessary model calls when output is already English.
    ...
    def invoke_translation(prompt):
        try:
            ...
            active = available.get("active", getattr(manager, "active", "none"))
            api_available = available.get("api", {}).get("available", False)
            local_available = available.get("local", {}).get("available", False)

            if active == "api" and api_available:
                return manager.invoke_api_only(prompt)
            if active == "local" and local_available:
...
def process_request(message, use_sidebar_values, sidebar_scores, conversation_history, model_config=None, language=None):
    ...
    def invoke_llm(prompt):
        ...
        if use_api:
            available_models = brain.llm_manager.get_available_models()
            if available_models.get("api", {}).get("available", False):
                # Enforce API usage when API mode is selected.
                return brain.llm_manager.invoke_api_only(prompt)
            if available_models.get("local", {}).get("available", False):
